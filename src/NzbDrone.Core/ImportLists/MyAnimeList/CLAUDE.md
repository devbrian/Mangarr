...
- **D-01 Sonarr-canonical plaintext token storage**: AccessToken / RefreshToken / Expires / AuthUser / PendingPkceState / ClientSecret stored as plain `string` / `DateTime` on the Settings JSON column. Zero crypto-at-rest infrastructure (no key-protection provider, no platform secret-store bridge, no per-row sealed blob). UI hides the token block via `Hidden = HiddenType.Hidden` only (Sonarr-canonical TraktSettings.cs:27-37); user-visible Password fields (ClientSecret) use `Privacy = PrivacyLevel.Password` so the FE renders `<input type=password>` and the V5 controller redacts the value on the outbound JSON response. V5 controller `ProviderControllerBase` strips Hidden fields from outbound schema JSON.
