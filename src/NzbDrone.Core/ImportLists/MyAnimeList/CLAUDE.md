...
| `MalImportListSettings.cs` | Settings POCO. Implements `IOAuthImportListSettings` per Plan 27-01 base contract. User-visible fields: `ClientId` (index 0, `FieldType.Textbox`) + `Status` enum (index 1, `FieldType.Select`). Hidden token block: `AccessToken`/`RefreshToken`/`Expires`/`PendingPkceState`/`AuthUser` (indices 2-6 with `Hidden=HiddenType.Hidden` only — Sonarr-canonical TraktSettings pattern). `SignIn` at index 7 (`FieldType.OAuth`, "Connect" button). `PendingPkceState` is a JSON-serialized `MalOAuthState` blob held as `string` per Discretion #2 shape (a). Per-user client model — user registers their own OAuth app at https://myanimelist.net/apiconfig and pastes the issued client_id (no client_secret — MAL public-client PKCE replaces the shared-secret defense). Consistent with MangaDex + AniList per-user client shapes in this phase. |
...
| `MalImportListProxy.cs` | OAuth + manga-list HTTP proxy with 4 methods: `BuildAuthorizeUrl(clientId, state)` (deterministic URL construction; no HTTP call), `ExchangeCodeForToken(clientId, code, verifier)` (POST form-urlencoded grant_type=authorization_code), `RefreshAccessToken(clientId, refreshToken)` (POST form-urlencoded grant_type=refresh_token), `GetUserMangaList(settings, nextCursor)` (GET /v2/users/@me/mangalist with Bearer auth + paging.next cursor support). `clientId` is the user-supplied value from `MalImportListSettings.ClientId`. Every outbound request flows through `ApplySharedHeaders()` which sets `RateLimitKey="myanimelist"` (NEW SourceKey bucket per CONTEXT line 36) + honest Mangarr UA + `Accept: application/json`. T-V7: NEVER logs code/verifier/token values. |
...
- **MAL ClientId is user-supplied per-install** (Phase 27 post-merge design correction — originally specced as Mangarr-pinned per Sonarr-Trakt precedent, but switched to per-user model for consistency with MangaDex + AniList in this same phase, and to avoid Mangarr maintainers owning an upstream MAL OAuth app registration + shared rate-limit budget across all installs). Users register their own OAuth app at https://myanimelist.net/apiconfig and paste the issued client_id into Settings. MAL public-client PKCE flow has NO client_secret — the verifier/challenge replaces the shared-secret defense.
...
Each Mangarr user supplies their own MAL OAuth `client_id` via
`MalImportListSettings.ClientId`. Mangarr does NOT ship a compiled-in
upstream-registered client. To set up the MAL provider in Settings:
...
3. Paste the issued `client_id` into the Mangarr Settings → ImportLists →
   MyAnimeList → Client ID field.
4. The MAL ClientId is NOT a secret per OAuth2 public-client semantics
   (PKCE replaces the shared-secret defense). MAL public-client PKCE has
   no client_secret. Storing the client_id on the Settings JSON column is
   the canonical per-user model — mirrors MangaDex + AniList shapes in
   this same phase.
...
|-----------|------------|
| T-27-04-V4 (token leakage to FE) | `[FieldDefinition(Hidden = HiddenType.Hidden)]` on AccessToken / RefreshToken / Expires / AuthUser / PendingPkceState (5 fields) — Sonarr-canonical TraktSettings pattern. V5 controller strips Hidden fields from outbound schema JSON. |
...
| T-27-04-V14 (ClientId user-supplied vs compiled-in) | Per-user `MalImportListSettings.ClientId` — each Mangarr install owns its own MAL OAuth app at https://myanimelist.net/apiconfig. ClientId is NOT a secret in OAuth2 public-client semantics (PKCE replaces the shared-secret defense). Consistent with MangaDex + AniList per-user client shapes. |
