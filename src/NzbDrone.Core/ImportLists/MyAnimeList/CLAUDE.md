...
| `MalImportListSettings.cs` | Settings POCO. Implements `IOAuthImportListSettings` per Plan 27-01 base contract. User-visible field: `Status` enum (index 0, `FieldType.Select`). Hidden token block: `AccessToken`/`RefreshToken`/`Expires`/`PendingPkceState`/`AuthUser` (indices 1-5 with `Hidden=HiddenType.Hidden` only — Sonarr-canonical TraktSettings pattern; `Privacy = PrivacyLevel.Password` was removed from Hidden fields in Phase 27 close-out fix-forward). `SignIn` at index 6 (`FieldType.OAuth`, "Connect" button). `PendingPkceState` is a JSON-serialized `MalOAuthState` blob held as `string` per Discretion #2 shape (a). No ClientId/ClientSecret fields — MAL ClientId is Mangarr-pinned compiled-in (`MalConstants.ClientId`) per public-client PKCE semantics. |
