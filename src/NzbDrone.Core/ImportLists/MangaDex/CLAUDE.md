...
**Path**: `src/NzbDrone.Core/ImportLists/MangaDex/`
...
| `MangaDexImportListSettings.cs` | Settings POCO. Implements `IOAuthImportListSettings` per Plan 27-01 base contract. User-visible credential block (`ClientId`/`ClientSecret`/`Username`/`Password`, indices 0-3 with `Privacy = PrivacyLevel.Password` on `ClientSecret`/`Password`) precedes the hidden token block (`AccessToken`/`RefreshToken`/`Expires`/`AuthUser`, indices 4-7 with `Hidden = HiddenType.Hidden` only — Sonarr-canonical TraktSettings pattern; `Privacy = PrivacyLevel.Password` was removed from Hidden fields in Phase 27 close-out fix-forward because it broke the new-Add round-trip per [27-05-SUMMARY addendum]). `SignIn` field at index 8 is `FieldType.OAuth` ("Test & Connect" button). |
...
|-----------|------------|
| T-27-02-V4 (token leakage to FE) | `[FieldDefinition(Hidden = HiddenType.Hidden)]` on AccessToken/RefreshToken/Expires/AuthUser fields (Sonarr-canonical TraktSettings pattern). V5 controller strips Hidden fields from outbound schema JSON. |
