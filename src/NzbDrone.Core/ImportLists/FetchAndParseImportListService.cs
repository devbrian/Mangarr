...
namespace NzbDrone.Core.ImportLists
{
    ...
    // .planning/reference/sonarr-vertical-slices/import-lists/FetchAndParseImportListService.cs
    // with manga-shape swaps:
    //   * fetchResult.Series → fetchResult.Manga (ImportListFetchResult property rename).
    //   * importListItemService.SyncSeriesForList → SyncMangaForList (peer rename).
    //   * Final dedup DistinctBy key: (TvdbId, ImdbId, Title) → manga-ID triplet
    ...
    public class FetchAndParseImportListService : IFetchAndParseImportList
    {
        ...
        public ImportListFetchResult Fetch()
        {
                ...
                var task = taskFactory.StartNew(() =>
                     {
                         try
                         {
                             ...
                             var importListReports = fetchResult.Manga;
                             ...
                             lock (result)
                             {
                                 ...
                                 if (!fetchResult.AnyFailure)
                                 {
                                     ...
                                     result.Manga.AddRange(importListReports);
                                     var removed = _importListItemService.SyncMangaForList(importListReports, importList.Definition.Id);
            ...
            // Sonarr divergence (Phase 26 Plan 26-04): final dedup key swapped from
            // (TvdbId, ImdbId, Title) to manga-ID triplet (MangaDexId, MalId, AniListId,
            // Title) — same Title-collision-collapse semantics, manga schema.
            result.Manga = result.Manga.DistinctBy(r => new { r.MangaDexId, r.MalId, r.AniListId, r.Title }).ToList();

            _logger.Debug("Found {0} total reports from {1} lists", result.Manga.Count, importLists.Count);
