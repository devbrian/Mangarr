    ...
    public class AutoTaggingSpecificationConverter : JsonConverter<List<IAutoTaggingSpecification>>
    {
        ...
        public override List<IAutoTaggingSpecification> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            while (reader.TokenType == JsonTokenType.StartObject)
            {
                ...
                // Phase 15 rebrand divergence — assembly is `Mangarr.Core` (was `Sonarr.Core` in
                // 6f857ba0e^ baseline). Type.GetType requires the FQ-name + assembly pair to
                // resolve the spec impl reflectively.
                var type = Type.GetType($"NzbDrone.Core.AutoTagging.Specifications.{typename}, Mangarr.Core", true);
