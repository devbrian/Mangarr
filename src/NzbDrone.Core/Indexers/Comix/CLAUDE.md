...
| `ComixPuppeteerSigner.cs` | Phase 17 D-05 concrete impl. PuppeteerSharp 24.42.0 + embedded headless Chromium; lazy-spawn warm page; behaviour-based namespace probe (`vmf_*` / response-interceptor detection per upstream `Signer.kt`); idle-teardown after 10 min (D-12); `IHandle<ApplicationShutdownRequested>` for clean teardown; layered LAZY REPROBE on EvaluateAsync error (B-2 path (a)); `SemaphoreSlim(1,1)` serializes per-request EvaluateAsync (D-06) with 5s drain on Dispose (W-2). DryIoc auto-discovered as singleton-via-interface per the existing `RegisterMany` convention (W-3). **Phase 17.2 D-1 (Mitigation A — settle-after-PROBE_JS):** `LaunchAndProbeAsync` inserts `WaitForNetworkIdleAsync(IdleTime=500ms, Timeout=5000ms)` between the `EvaluateExpressionAsync<ProbeResult>(PROBE_JS)` line and the probe-result validation block. The settle re-acquires the page execution context handle so subsequent `EvaluateExpressionAsync` calls in `EvaluateProxyFetchAsync` hit a stable context, NOT one mid-`Runtime.executionContextDestroyed` from PROBE_JS's pushState side-effect (per Phase 17 17-08-LIVE-VERIFICATION-EVIDENCE.md Finding 3 + 17-LEARNINGS.md L-1/S-2). Annotation block at lines 240-280 references `Phase 17.2 D-1`. PROBE_JS string literal byte-for-byte unchanged (D-14 / SC#5). Regression-guard: `ComixSignerProbeSameNamespaceFixture.Probe_must_settle_after_PROBE_JS_before_EvaluateProxyFetchAsync` (Chromium-free grep against production source). |
