...
**Architecture (current — 2026-05-23 env-module oracle):**
...
- **env-module oracle via dynamic import** (replaces the upstream-incomplete captureToken approach): on each `ProxyFetchAsync(apiPath)`:
  ...
  2. Call `EnsureEnvModuleAsync(page, pageUrl, ct)` — navigates if env module URL not yet cached, then sniffs the env module URL from `RequestFinished` events (filters for `env-tfgaak-*.js` files on `comix.to`). The captured URL is cached for 5 minutes (`BootstrapCacheTtl` — bundle URL is content-hashed so it's stable per build).
  ...
  4. Invoke from page context: `page.EvaluateFunctionAsync<string>("async (modUrl, p, paramsObj) => { const mod = await import(modUrl); const f = mod.f; const opts = paramsObj && Object.keys(paramsObj).length > 0 ? { params: paramsObj } : undefined; const res = await f.get(p, opts); return typeof res === 'string' ? res : JSON.stringify(res); }", envModuleUrl, pathPart, paramsJson)`.
...
- The env module exports the bundle's pre-configured axios instance as `mod.p` and a `b`-wrapper around it as `mod.f`. `mod.f.get(path, {params})` returns the unwrapped response data (post the bundle's ok+result interceptor + post `Hi(ai)`'s decryption interceptor) — i.e., plaintext JSON.
