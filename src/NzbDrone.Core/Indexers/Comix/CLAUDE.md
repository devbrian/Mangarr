...
**Architecture (current — 2026-05-25 env-module oracle, URL filter rotated):**
...
- **env-module oracle via dynamic import** (replaces the upstream-incomplete captureToken approach): on each `ProxyFetchAsync(apiPath)`:
  ...
  2. Call `EnsureEnvModuleAsync(page, pageUrl, ct)` — navigates if env module URL not yet cached, then sniffs the env module URL from `RequestFinished` events (filters for `env-tfkr3g-*.js` files on `comix.to`; rotated from `env-tfgaak-*.js` on 2026-05-25 — Phase 33.1). The captured URL is cached for 5 minutes (`BootstrapCacheTtl` — bundle URL is content-hashed so it's stable per build).
  ...
  4. Invoke from page context: `page.EvaluateFunctionAsync<string>("async (modUrl, p, paramsJson) => { const mod = await import(modUrl); const f = mod.g; const paramsObj = paramsJson ? JSON.parse(paramsJson) : {}; const opts = Object.keys(paramsObj).length > 0 ? { params: paramsObj } : undefined; const res = await f.get(p, opts); return typeof res === 'string' ? res : JSON.stringify(res); }", envModuleUrl, pathPart, paramsJson)`. (Phase 33.1 2026-05-25: accessor rotated `mod.f` → `mod.g`.)
...
- The env module exports the bundle's `.get→.data` axios wrapper (internal `N`) as `mod.g` (was `mod.f` pre-2026-05-25; export names re-mangle per build) and the raw axios instance under another rotating export. `mod.g.get(path, {params})` returns the unwrapped response data (post the bundle's ok+result interceptor + post `Hi(ai)`'s decryption interceptor) — i.e., plaintext JSON.
