...
- **captureToken via request interception** (replaces the upstream-obsolete PROBE_JS namespace-probe approach): on each `ProxyFetchAsync(apiPath)`:
  ...
  6. Relay the actual `/api/v1{apiPath}?_=<token>` GET through the page's same-origin `fetch()` — the page's Cloudflare session + cookies + UA apply (Choice A per debug doc).
