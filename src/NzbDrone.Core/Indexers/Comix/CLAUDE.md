...
| `ComixRequestGenerator.cs` | URL composition: `/api/v1/manga?order[chapter_updated_at]=desc` (FetchRecent) + `/api/v1/manga/{hid}/chapters` (search). When invoked with `ChapterSearchCriteria`, appends `&number={chapterNumber}` for server-side per-chapter filtering (verified live 2026-05-08: filter narrows the chapter-list response from ~97 rows to ~6 for chapter 1). The token derivation pins to the path only, so the filter does not invalidate the anti-bot signature. Hash key derived from `Manga.CleanTitle` → slug fallback (Manga model has no `ComixHash` field; v2 metadata-source linkage will replace with canonical `hash_id` lookup). |
