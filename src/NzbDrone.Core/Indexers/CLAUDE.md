...
| `Gateway/` | Manga gateway | `GatewayIndexer` — the sole `IIndexer` (Phase 37; external manga gateway client). **SEARCH is multi-page since quick task 260620-ing** (offset validated live 2026-06-20): offset stride (`0, L, 2L, …`, `L = ResultLimit` per-page pull, un-clamped) walked by the kept `HttpIndexerBase` paging engine via a dynamic `PageSize => GatewayRequestGenerator.ResolveEffectiveLimit(...)` (single source of truth, mirrors the MangaDex import-list precedent), stopping at the first short page. **Paging splits on `Interactive`:** automatic search (RSS/missing/monitored) walks FULL coverage (bounded by `MaxSearchPages` runaway guard; `MaxNumResultsPerQuery` lifted to `int.MaxValue` so a >1000-row page isn't truncated), while the interactive Search tab emits a SINGLE first page (the "only show first page" cap). `/recent` remains single-request. See `DIVERGENCE.md`. |
