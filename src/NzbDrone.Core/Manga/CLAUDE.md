...
| `IChapterSynthesisService.cs` + `ChapterSynthesisService.cs` | **NEW-in-Mangarr (Phase 40 Plan 40-03; no Sonarr analog — see [DIVERGENCE.md § Phase 40](../../../DIVERGENCE.md)).** Synthesizes Chapter rows from external-gateway releases so the catalog reflects grabbable reality (the MangaDex catalog and the gateway are two separate sources). `SynthesizeFromDecisions(manga, decisions)` (on-search) backfills the genuinely-missing whole-number range `[1..maxWhole]` (plus Chapter 0 only when a chapter-0 release is attributed); `SynthesizeForGrab(remoteChapter)` (on-grab) adds the single grabbed number. Precision-first attribution gate per release: ID-match (`ReleaseInfo.Ids` cross-source dict) **else** EXACT-title — the inexact substring strategy is EXCLUDED (D-06/D-07 cross-title-corruption guard). Monitor flag computed from `Manga.MonitorNewItems` (All→Monitored, None→unmonitored; D-05), NOT via `ChapterMonitoredService`. Writes only genuinely-absent numbers through `IChapterListService.SyncChapters` (matches on (MangaId, ChapterNumber), never clobbers a real Title, never deletes — D-08). Bounded `maxWhole` range cap (D-03 DoS guard). Auto-registered by DryIoc; call sites: `MangaReleaseSearchService.MangaSearch` (on-search, Plan 40-04), `MangaReleaseController.DownloadRelease` (on-grab, Plan 40-04), and `MangaRssSyncService.Execute` (on-`/recent` RSS self-heal — groups RSS decisions by resolved `RemoteChapter.Manga` and calls `SynthesizeFromDecisions` per manga before the grab pipeline, so a brand-new chapter that only ever appears in the gateway's recent feed becomes wanted instead of being silently dropped as an empty-`Chapters` decision; `SynthesizeFromDecisions` returns the new-row count, and the RSS path re-runs `GetRssDecision` for a same-tick grab ONLY when that count is non-zero). |
