...
| File | Purpose |
|------|---------|
| `MangaDexMetadataSource.cs` | Provider impl. Extends `HttpMetadataSourceBase<MangaDexMetadataSourceSettings>`. `DefaultIsPrimary => false` (was `true` per D-16; flipped Phase 41 / Plan 41-03 D-01a — MangaBaka is the new default primary). `MangaTitleNormalizer.Normalize` applied to user-typed search title before query. |
