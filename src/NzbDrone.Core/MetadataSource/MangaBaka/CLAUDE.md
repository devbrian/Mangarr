...
| `Resource/MangaBakaSeries.cs` | Rich series record + `source` (AniList/MAL with RAW INTEGER `id`), `cover.raw.url` (pre-resolved poster URL), named title fields, `total_chapters` (JSON STRING). |
...
- **Direct cross-source ids (D-03 / D-08-R):** `record.Source?.AniList?.Id` / `record.Source?.MyAnimeList?.Id` are RAW INTEGERS read straight into `Manga.AniListId` / `Manga.MalId` — no `int.TryParse` on a string (the MangaDex Pitfall 7 path), and short-circuiting `CrossSourceIdResolver` entirely (T-41-XID). `MangaDexId` stays **null** (D-03a): a MangaBaka-sourced manga is not a MangaDex record, so grab attribution leans on the AniList/MAL axes.
