...
public class PalettePresenceFixture : AutomationTest
{
    ...
    public async Task manga_pink_logo()
    {
        ...
        // WR-11 (18-REVIEW): hex assertions must be case-insensitive (an SVG
        // regenerated by a tool that uppercases hex would otherwise fail
        // despite the palette being correct) and must guard against the
        // 3-digit short form's 6-digit expansion (#00CCFF renders
        // identically to #0CF — the prior NotContain on "#0CF" would not
        // catch a tool that expanded the short form on regenerate).
        var svgLower = svg.ToLowerInvariant();

        // STATE assertion 2: canonical manga-pink hex is present (case-insensitive).
        svgLower.Should().Contain(
            MangaPinkHex.ToLowerInvariant(),
            "because logo.svg must carry the rebranded manga-pink palette (PageHeader.tsx L46-L51)");

        // STATE assertion 3: Sonarr cyan accent was retinted away — guard
        // BOTH the 3-digit short form (#0CF) and the 6-digit expansion
        // (#00CCFF).
        svgLower.Should().NotContain(
            SonarrCyanHex.ToLowerInvariant(),
            "because the Sonarr cyan accent (#0CF) was retinted to manga pink in the Phase 15 D-A close-out");
        svgLower.Should().NotContain(
            "#00ccff",
            "because the 6-digit equivalent of #0CF renders identically and would also indicate a missed retint");
    }
}
