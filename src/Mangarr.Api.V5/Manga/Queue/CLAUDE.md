...
| `MangaQueueResource.cs` (+ `MangaQueueResourceMapper`) | Queue DTO; flattens entity → wire shape; does not leak `RemoteChapter` (in-process EF reference); single-arg `ToResource()` extension shared with `MangaQueueDetailsController`. Carries `SourceKey` (the gateway SourceKey from `ReleaseInfo.Source`, e.g. mangadex / comix.to) alongside `Indexer` — the manga-useful per-release identifier now that `GatewayIndexer` is the sole indexer and `Indexer` is always "Gateway"; mirrors `ChapterHistoryResource.SourceKey` / `MangaBlocklistResource.SourceKey`. Plain string → serialized verbatim (no `FirstCharToLower`, per GH #307 which only camelCases the enum-ToString status fields). |
