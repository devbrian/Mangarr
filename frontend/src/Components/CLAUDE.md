...
Reusable, domain-agnostic UI components used across the app. Inherited from
Sonarr and reused as-is — the only Mangarr-specific surface here is
`SignalRListener.tsx` (the manga resource → React Query cache dispatch table,
documented below) plus the few manga-named tag widgets (`MangaTagList.tsx`).

## Layout

Sub-dirs: `Page/` (+ `Toolbar/`, `Header/`, `Sidebar/`), `Table/` (incl.
`VirtualTable*`), `Form/`, `Modal/`, `Menu/`, `Filter/`, `FileBrowser/`,
`Link/`, `Loading/`, `Error/`, `DescriptionList/`, `Select/`, `Router/`,
`Scroller/`, `Markdown/`, `Tooltip/`. Single-file components live at the dir
root (`Alert.tsx`, `Card.tsx`, `Icon.tsx`, `Label.tsx`, `FieldSet.tsx`,
`ProgressBar.tsx`, `CircularProgressBar.tsx`, `MonitorToggleButton.tsx`,
`HeartRating.tsx`, `TagList.tsx`, `MangaTagList.tsx`, …). Each `.tsx` pairs with
a hand-checked-in `<name>.css` + `<name>.css.d.ts` (NOT `.module.css`).

`Form/` inputs: `TextInput`, `NumberInput`, `FloatInput`, `CheckInput`,
`SelectInput`, `PathInput`, `PasswordInput`, `TextArea`, `AutoCompleteInput`,
`AutoSuggestInput`, `CaptchaInput`, `OAuthInput`, `KeyValueListInput`, plus the
`FormGroup` / `FormLabel` / `FormInputGroup` scaffolding. (No `QualityProfile*`
input — Phase 5 D-04 dropped the TV quality model.)

Icons come from `Helpers/Props` (`<Icon name={icons.DELETE} />`).
