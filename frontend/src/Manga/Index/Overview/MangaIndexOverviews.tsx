...
import MangaIndexOverview from './MangaIndexOverview';
...
interface MangaIndexOverviewsProps {
  items: Manga[];
  sortKey: string;
...
function Row({ index, style, data }: ListChildComponentProps<RowItemData>) {
  ...
  const manga = items[index];

  return (
    <div style={style}>
      <MangaIndexOverview mangaId={manga.id} {...otherData} />
    </div>
  );
}
...
function MangaIndexOverviews(props: MangaIndexOverviewsProps) {
  const {
    items,
    sortKey,
    jumpToCharacter,
  ...
  const { size: posterSize, detailedProgressBar } = useMangaOverviewOptions();
...
export default MangaIndexOverviews;
