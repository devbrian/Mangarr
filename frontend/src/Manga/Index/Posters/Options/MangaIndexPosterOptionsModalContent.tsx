...
interface MangaIndexPosterOptionsModalContentProps {
  onModalClose(...args: unknown[]): unknown;
}

function MangaIndexPosterOptionsModalContent({
  onModalClose,
}: MangaIndexPosterOptionsModalContentProps) {
  const {
    detailedProgressBar,
    size,
    showTitle,
  ...
  const onPosterOptionChange = useCallback(
    ({ name, value }: { name: string; value: unknown }) => {
      setMangaPosterOptions({ [name]: value });
    },
    []
  );
...
export default MangaIndexPosterOptionsModalContent;
