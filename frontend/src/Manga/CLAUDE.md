...
- **7-value `MangaMonitor`** — #357 D-2 unified the import-list monitor enum onto `MangaMonitor`, extending it to `'all' | 'future' | 'missing' | 'existing' | 'first' | 'latest' | 'none'` (supersedes the Phase 6 D-03 5-value lock). Do NOT extend this without a documented decision.
