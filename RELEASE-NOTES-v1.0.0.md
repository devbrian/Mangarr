...
- **Docker image ships `linux/amd64` only at v1.0.0.** PuppeteerSharp's `BrowserFetcher` (the Chromium bake mechanism behind the Comix runtime signer per Phase 17 D-02) downloads from the Chrome for Testing release channel, which does not publish an arm64 Linux build (verified upstream: `https://googlechromelabs.github.io/chrome-for-testing/known-good-versions-with-downloads.json` lists Chrome platforms as `linux64 | mac-arm64 | mac-x64 | win32 | win64` only). The chromium-builder stage's arch-aware dispatch is wired and ready, but until v1.1-17 lands a real arm64 headless-browser path — Playwright .NET CLI bundles arm64 chromium, or hybrid apt-chromium relaxing D-02 — the runtime image cannot ship a working arm64 variant. **Workaround for arm64 hosts (Raspberry Pi 4/5, Apple Silicon, AWS Graviton, etc.):** Docker's user-mode emulation pulls the amd64 image cleanly: `docker pull --platform linux/amd64 ghcr.io/devbrian/mangarr:1.0.0`. Performance penalty is real but tolerable for a manga library manager.
